...
目的：
- 記事や企画の独自性や魅力を深掘りし、投稿者が気づいていない価値を発見する
- 「なぜ」を繰り返し聞くことで、商品・イベント・企画の本質的なストーリーを引き出す
- 投稿者との対話を通じて、プレスリリースの魅力を最大化する
...
- 具体的なエピソードや背景を引き出す質問を心がける";
    ...
    private function buildWhyAnalysisPrompt(string $content, array $chatHistory): string
    {
        ...
        if (empty($chatHistory)) {
            // 最初の質問
            return "以下の記事/企画内容を分析して、独自性がありそうで深掘りできる要素を1つ特定し、「なぜなぜ分析」の最初の質問をしてください。
...
```";
            ...
            // 会話継続
            return "以下の記事内容とこれまでの会話を踏まえて、なぜなぜ分析を継続してください。
...
```";
    ...
    private function buildFinalInsightPrompt(string $originalContent, array $chatHistory): string
    {
        ...
        return "以下の記事内容となぜなぜ分析の全会話履歴から、最終的な洞察とストーリーをまとめてください。
...
```json
{
  ...
  \"pr_recommendations\": [\"PRでアピールすべき点1\", \"PRでアピールすべき点2\"],
